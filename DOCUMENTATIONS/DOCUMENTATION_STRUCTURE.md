...
Analyzing Results?
  └─> scripts/README_REPLICATION.md
        ...
        ├─> Accuracy data ─> make_stats_table.py --stats acc
        └─> Length stats ─> make_stats_table.py --stats length
...
Tool: make_stats_table.py (from research repo)
├─ Input: outputs/inference/
├─ Output: Console tables (accuracy, length)
├─ Usage:
│  ├─ python -m make_stats_table --stats acc
│  └─ python -m make_stats_table --stats length
└─ Used: After experiments

Tool: Quantization scripts (from research repo)
├─ Location: scripts/quantization/
├─ Methods: AWQ, GPTQ, KVQuant*, QuaRot-KV, 
│           SmoothQuant, QuaRot, FlatQuant
├─ Output: outputs/modelzoo/
└─ Used: Main quantization phase

Tool: Inference scripts (from research repo)
├─ Location: scripts/inference/
├─ Input: Model path, GPU device
├─ Output: outputs/inference/ (JSONL results)
└─ Used: Evaluation phase

Tool: nvidia-smi (system utility)
├─ Purpose: GPU monitoring
├─ Output: GPU status (util, memory, power, temp)
└─ Usage: nvidia-smi or watch -n 1 nvidia-smi

Documentation Templates:
├─ REPLICATION_OUTPUTS.md
│  └─ Fill manually with experiment results
└─ RESOURCE_BENCHMARKING.md
   └─ Fill manually with resource metrics
```
